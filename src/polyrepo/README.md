...
The package reads a `repositories.yaml` manifest, builds a reusable frozen base
...
1. Declare the workspace topology in `repositories.yaml`.
...
- A `repositories.yaml` file at the workspace root.

## Manifest

Create `repositories.yaml` at the root of the workspace that coordinates the
member repositories.
...
- Store credentials outside `repositories.yaml`, setup templates, and launcher
  scripts.
...
Prefer adding new launcher-specific behavior outside this package until it
